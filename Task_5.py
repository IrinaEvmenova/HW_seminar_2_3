...
# отрицательных индексов. (Дополнительное)
...
def negativ_fibonachi(n: int) -> int:
    '''Возвращает член последовательности негофибоначчи'''
    return (-1)**(n+1)*(fibonachi(n))

def secuence_of_fibs(n: int) -> list[int]:
    '''Возвращает последовательность Фибоначчи'''
    list1 = [negativ_fibonachi(i) for i in range(n+1)][::-1]
    list2 = [fibonachi(i) for i in range(1, n+1)]
    return list1 + list2

n = 5
print(secuence_of_fibs(n))